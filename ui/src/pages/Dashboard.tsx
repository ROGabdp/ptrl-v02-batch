        ...
        async function fetchData() {
            try {
                const [best, runs, bts, jobs] = await Promise.all([
                    ...
                    api.jobs.getRecent({ limit: 5 }).catch(() => []),
                ])
                setBestModels(best)
                setRecentRuns(runs)
                ...
                <section className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {bestModels.slice(0, 4).map((model) => {
                            const actionsHref = `/actions?type=backtest&ticker=${encodeURIComponent(model.ticker)}&model_path=${encodeURIComponent(model.model_path)}`
                            return (
                                <div
                                    key={model.ticker}
                                ...
                                >
                                    ...
                                    <Link
                                        to={actionsHref}
                                        className="mt-2 block w-full text-center px-2 py-1 border border-indigo-200 text-xs font-medium rounded text-indigo-700 bg-indigo-50 hover:bg-indigo-100"
                                    >
                                        Open in Actions
                                    </Link>
                                </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-100 divide-y divide-gray-100">
                        {recentJobs.map((job) => (
                            <Link key={job.job_id} to={`/jobs/${job.job_id}`} className="block p-4 hover:bg-gray-50 transition-colors">
                                ...
                                <div className="text-sm text-gray-600">{job.job_type} · {job.command.slice(0, 5).join(' ')}</div>
                            </Link>
                        ))}
